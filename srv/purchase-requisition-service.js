...
async function validateForeignKeys(tx, data, context = "") {
    ...
    if (data.storageLocation_storageLocation || data.storageLocation) {
        const storageLocationCode = data.storageLocation_storageLocation || data.storageLocation;
    ...
    if (data.purchaseRequisitionType) {
        ...
        const prType = await tx.run(
            SELECT.one
                .from(PurchasingDocumentTypes)
                .where({ documentCategory: 'B', documentType: data.purchaseRequisitionType })
        );
        if (!prType) {
...
function validateDataFormat(data, context = "") {
    ...
    if (data.deliveryDate) {
        ...
        if (isNaN(deliveryDate) || deliveryDate < currentDate) {
            return `Delivery Date must be a future date (≥ ${currentDate.toISOString().split('T')[0]}) (${context})`;
        }
    }
...
async function validatePurchasingLimit(tx, data, existingPR, context = "") {
  ...
  const purchasingOrg = data.purchasingOrganization ?? existingPR?.purchasingOrganization;
 
  if (!quantity || quantity <= 0) return null;
  
 
  const infoRecord = await tx.run(
    SELECT.one.from('transaction.table.PurchasingInfoRecord').where({ MaterialMaster })
  );
  ...
  const eine = await tx.run(
    SELECT.one.from('transaction.table.PurchasingOrganizationData').where({
  ...
  if (!eine || eine.netPrice == null || eine.priceUnit == null) {
    return `No org-level pricing found for InfoRecord '${infoRecord.purchasingInfoRecord}' and Org '${purchasingOrg}' (${context})`;
  }
  ...
  if (totalValue > BUDGET_LIMIT) {
    return `Total value (${totalValue.toFixed(2)} AUD) exceeds budget (${BUDGET_LIMIT.toLocaleString()} AUD) (${context})`;
  }
...
srv.before("UPDATE", PurchaseRequisition, async (req) => {
  ...
  srv.before("release", PurchaseRequisition, async (req) => {
    const { purchaseRequisition, purchaseReqnItem } = req.params[0];
    const pr = await SELECT.one
      .from(PurchaseRequisition)
      .where({ purchaseRequisition, purchaseReqnItem });
    ...
    if (pr.releaseStatus !== "Approved") {
      req.error(
        400,
        `Action Release is not available for releaseStatus: ${pr.releaseStatus}`
      );
    }
  });

  srv.on("release", PurchaseRequisition, async (req) => {
    const { purchaseRequisition, purchaseReqnItem } = req.params[0];
    await UPDATE(PurchaseRequisition)
